...
# test_df = test_df[0:10000]
# val_df = val_df[0:10000]
...
# train_dataset, discard = torch.utils.data.random_split(train_data, [int(len(train_data)*0.1), int(len(train_data)*0.9)+1])
...
def train_model():
    ...
    #         break
            ...
            #ADD metadata for checkpoint
            if (aucs[0] + aucs[1]+ aucs[2])/3 > best_val_auc:
                best_val_auc = (aucs[0] + aucs[1]+ aucs[2])/3
